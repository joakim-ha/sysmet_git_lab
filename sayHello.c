#include <stdio.h>

int main(void)
{
    printf("Hello from Joakim!\n");
    printf("Hello from Rasmus?!\n");
    printf("Hello from John!");
    printf("Hello from Josefine!");
    printf("Hello from Cecilia!!");
    printf("Hello from Jonathan");
    printf("Hello from Lucian\n");
    printf("Hello from Felix!\n");
    printf("Hello from Reza!");
    printf("Hello from Dmitri!\n");
    printf("Hello fron Alexandra");
    printf("Hello from Christian\n");
    printf("Hello from Tobias\n");
    printf("Hello from Sofie!\n");
    printf("Hello from Maja!\n");
    return 0;
}
